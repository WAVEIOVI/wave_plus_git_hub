...
return new class extends Migration
{
    ...
    {
        Schema::create('eq_servicings', function (Blueprint $table) {
            $table->id();
            // Core Identification
            $table->string('eq_servicing_id')->unique();
            $table->json('name', 255);
            ...
            $table->decimal('tax_sale', 12, 3)->nullable()->comment('The sale price of the eq_servicing with tax.');
            ...
            $table->enum('eq_servicing_type', ['inspection and testing', 'maintenance and repair', 'refill recharge', 'calibration and certification', 'installation decommissioning'])->nullable();
            $table->enum('eq_servicing_status', ['active', 'draft', 'inactive'])->default('draft');
            $table->softDeletes();
            $table->timestamps();
        });
    }
    ...
    {
        Schema::dropIfExists('eq_servicings');
    }
};
