...
import { getEqServicingTypes } from '@/constants/filterOptions'
...
watch(
  () => fetchedEqServicing.value.category_id,
  newCategoryId => {
...
const onFormSubmit = () => {
  emit('addEqServicing', fetchedEqServicing.value)
  resetForm()
}
...
<template>
  ...
  >
    ...
    <!-- Dialog Content -->
    <VCard :title="$t('Add New Equipment Servicing')">
      <VForm
        class="mt-4"
      ...
      >
        <VCardText>
          <VRow>
            <VCol cols="12">
              <AppSelect
                v-model="fetchedEqServicing.eq_servicing_type"
                :label="$t('Equipment Servicing Type')"
                density="compact"
            ...
            <VCol cols="12">
              <AppTextField
                v-model="fetchedEqServicing.name[$i18n.locale]"
            ...
            <VCol cols="12">
              <AppSelect
                v-model="fetchedEqServicing.category_id"
                :label="$t('Category')"
                density="compact"
                :items="categories"
                item-title="title"
            ...
            <VCol cols="12">
              <AppSelect
                v-model="fetchedEqServicing.subcategory_id"
                :label="$t('Subcategory')"
                density="compact"
                :items="subcategories"
                item-title="title"
