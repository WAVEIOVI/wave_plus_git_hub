...
watch(
  () => fetchedEqBlueprint.value.category_id,
  newCategoryId => {
...
const onFormSubmit = () => {
  emit('addEqBlueprint', fetchedEqBlueprint.value)
  resetForm()
}
</script>
...
<template>
  ...
  >
    ...
    <!-- Dialog Content -->
    <VCard :title="$t('Add New Equipment Blueprint')">
      <VForm
        class="mt-4"
      ...
      >
        <VCardText>
          <VRow>
            <VCol cols="12">
              <AppTextField
                v-model="fetchedEqBlueprint.name[$i18n.locale]"
            ...
            <VCol cols="12">
              <AppSelect
                v-model="fetchedEqBlueprint.category_id"
                :label="$t('Category')"
                density="compact"
                :items="categories"
                item-title="title"
            ...
            <VCol cols="12">
              <AppSelect
                v-model="fetchedEqBlueprint.subcategory_id"
                :label="$t('Subcategory')"
                density="compact"
                :items="subcategories"
                item-title="title"
