...
import { getConsumableTypes } from '@/constants/filterOptions'
...
watch(
  () => fetchedConsumable.value.category_id,
  newCategoryId => {
...
const onFormSubmit = () => {
  emit('addConsumable', fetchedConsumable.value)
  resetForm()
}
...
<template>
  ...
  >
    ...
    <!-- Dialog Content -->
    <VCard :title="$t('Add New Consumable')">
      <VForm
        class="mt-4"
      ...
      >
        <VCardText>
          <VRow>
            <VCol cols="12">
              <AppSelect
                v-model="fetchedConsumable.consumable_type"
            ...
            <VCol cols="12">
              <AppTextField
                v-model="fetchedConsumable.name[$i18n.locale]"
            ...
            <VCol cols="12">
              <AppSelect
                v-model="fetchedConsumable.category_id"
                :label="$t('Category')"
                density="compact"
                :items="categories"
                item-title="title"
            ...
            <VCol cols="12">
              <AppSelect
                v-model="fetchedConsumable.subcategory_id"
                :label="$t('Subcategory')"
                density="compact"
                :items="subcategories"
                item-title="title"
